...
function App() {
  ...
  async function addContactToList(listId, contact){ const already=expandedListContacts.some(c=>c.phone===contact.phone); if(already)return; const r=await api('/lists/'+listId+'/contacts',{method:'POST',body:JSON.stringify({contacts:[{phone:contact.phone,name:contact.name}]})},token); if(!r.error){ const d=await api('/lists/'+listId+'/contacts',{},token); setExpandedListContacts(Array.isArray(d)?d:[]); loadMyLists(); } }
    ...
    <div style={S.app}>
      ...
      {showForm&&<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&(setShowForm(false),setEditingMsg(null),setSelContact(null),setSelContacts([]),setMsgText(''),setSchedAt(''),setSearch(''),setShowManual(false),setManualName(''),setManualPhone(''))}>
        <div style={S.modal}>
          ...
          <input style={S.input} placeholder={recipientTab==='groups'?'Buscar grupo...':recipientTab==='mylists'?'Buscar lista...':'Buscar contacto...'} value={search} onChange={e=>setSearch(e.target.value)} onFocus={()=>setSearchFocused(true)} onBlur={()=>setSearchFocused(false)}/>
          <div style={{...S.contactList,display:(searchFocused||recipientTab==='mylists')?'block':'none'}}>
            {recipientTab==='mylists'
              ? myLists.length===0
                ? <div style={{padding:'14px',fontSize:13,color:'#aaa'}}>Sin listas — créalas con el botón 📋 Listas</div>
                : myLists.filter(l=>l.name.toLowerCase().includes(search.toLowerCase())).map(l=>(
                    <div key={l.id} style={{padding:'10px 14px',cursor:'pointer',borderBottom:'1px solid #f5f5f5',display:'flex',alignItems:'center',gap:8,userSelect:'none'}} onMouseDown={e=>{e.preventDefault();selectMyList(l.id);}}>
                      <span style={{fontSize:14}}>📋</span>
                      <div><div style={{fontWeight:500,fontSize:13}}>{l.name}</div><div style={{fontSize:11,color:'#888'}}>{l.count} contactos — click para seleccionar todos</div></div>
                    </div>
                  ))
              : (()=>{const norm=p=>p?p.replace(/\D/g,''):'';const usageCount=new Map();messages.forEach(m=>{const k=norm(m.phone);usageCount.set(k,(usageCount.get(k)||0)+1);});const byOrder=(a,b)=>{const aUse=usageCount.get(norm(a.phone))||0;const bUse=usageCount.get(norm(b.phone))||0;if(bUse!==aUse)return bUse-aUse;const lookup=p=>{const n=norm(p);return chatOrderMap?.get(n)??chatOrderMap?.get('56'+n)??9999;};return lookup(a.phone)-lookup(b.phone);};const base=recipientTab==='groups'?groups:contacts.filter(c=>c.source!=='whatsapp_group'&&c.source!=='whatsapp_broadcast');const icon=recipientTab==='groups'?'👥':'👤';const filtered=base.filter(c=>c.name.toLowerCase().includes(search.toLowerCase())||(c.phone||'').includes(search)).sort(byOrder).slice(0,20);return filtered.length===0?<div style={{padding:'10px 14px',fontSize:13,color:'#aaa'}}>{recipientTab==='groups'&&!groups.length?'Cargando grupos...':'Sin resultados'}</div>:filtered.map(c=>{const sel=selContacts.some(x=>x.phone===c.phone);return(<div key={c.phone||c.id} style={{padding:'10px 14px',cursor:'pointer',borderBottom:'1px solid #f5f5f5',display:'flex',alignItems:'center',gap:8,userSelect:'none',background:sel?'#f0fff4':undefined}} onMouseDown={e=>{e.preventDefault();setSelContacts(p=>sel?p.filter(x=>x.phone!==c.phone):[...p,c]);setSearch('');}}><span style={{fontSize:14,pointerEvents:'none'}}>{sel?'✅':icon}</span><div style={{pointerEvents:'none'}}><div style={{fontWeight:500,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:'#888'}}>{recipientTab==='contacts'?c.phone:'Grupo'}</div></div></div>);});})()}
          </div>
