    ...
    <div style={S.app}>
      ...
      {showForm&&<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&(setShowForm(false),setEditingMsg(null),setSelContact(null),setSelContacts([]),setMsgText(''),setSchedAt(''),setSearch(''),setShowManual(false),setManualName(''),setManualPhone(''))}>
        <div style={S.modal}>
          ...
          {selContacts.length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:6,alignItems:'center'}}>{selContacts.map(c=><span key={c.phone} style={{background:'#e8f5e9',color:'#1b5e20',fontSize:12,fontWeight:600,padding:'4px 10px',borderRadius:20,display:'flex',alignItems:'center',gap:4}}>{c.name}<span style={{cursor:'pointer',fontWeight:400,marginLeft:2}} onMouseDown={e=>{e.preventDefault();setSelContacts(p=>p.filter(x=>x.phone!==c.phone));}}>✕</span></span>)<button style={{...S.btnP,fontSize:12,padding:'4px 12px',marginLeft:'auto'}} onMouseDown={e=>{e.preventDefault();setShowList(false);setSearch('');}}>Listo ✓</button></div>}
          {showList&&<input style={{...S.input,marginBottom:4}} placeholder={recipientTab==='groups'?'Buscar grupo...':recipientTab==='lists'?'Buscar lista...':'Buscar contacto...'} value={search} onChange={e=>setSearch(e.target.value)}/>}
          {showList&&<div style={S.contactList}>
          ...
          {!showList&&<button style={{...S.btnSm,marginBottom:6,fontSize:12}} onMouseDown={e=>{e.preventDefault();setShowList(true);}}>+ Agregar más</button>}
