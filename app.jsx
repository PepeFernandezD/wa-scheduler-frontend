    ...
    <div style={S.app}>
      ...
      {showLists&&<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowLists(false)}>
        <div style={{...S.modal,maxHeight:'85vh',display:'flex',flexDirection:'column'}}>
          ...
          <button style={{...S.btnSm,marginBottom:12,justifyContent:'center',border:'1px dashed #25d366',color:'#25d366',background:'none',width:'100%'}} onClick={openImport}>⬇ Importar desde WhatsApp</button>
          <div style={{overflowY:'auto',flex:1}}>
            {myLists.map(l=>(
              <div key={l.id} style={{borderBottom:'1px solid #f0f0f0',marginBottom:2}}>
                ...
                {expandedListId===l.id&&<div style={{paddingBottom:10,paddingLeft:4}}>
                  ...
                  {listContactSearch.length>0&&<div style={{...S.contactList,marginBottom:8,maxHeight:120}}>
                    {contacts.filter(c=>c.source!=='whatsapp_group'&&(c.name.toLowerCase().includes(listContactSearch.toLowerCase())||c.phone.includes(listContactSearch))).slice(0,8).map(c=>(
          ...
          {showImport&&<>
            ...
            {waBroadcasts.length===0&&<div style={{color:'#aaa',fontSize:13,marginBottom:8}}>Cargando listas...</div>}
            {waBroadcasts.map(b=>(
